...
class MagnitudeWarping(object):
    ...
    def __init__(self, sigma=0.1, knot=4, p=0.5, wSize=500, channels=6, seed=SEED):
        ...
        if channels == 1:
            self.randomCurves = np.squeeze(
                np.array([CubicSpline(self.x[:, i], self.y[:, i])(self.x_range) for i in range(channels)]).transpose())
        else:
            self.randomCurves = np.array(
                [CubicSpline(self.x[:, i], self.y[:, i])(self.x_range) for i in range(channels)]).transpose()
...
class TimeWarping(object):
    ...
    def __init__(self, sigma=0.1, knot=4, p=0.5, wSize=500, channels=6, seed=SEED):
        ...
        self.tt = np.array([CubicSpline(self.x[:, i], self.y[:, i])(self.x_range)
                            for i in range(channels)]).transpose()
        self.tt_cum = np.cumsum(self.tt, axis=0)
        # set the shape
        self.t_scale = [(wSize - 1) / self.tt_cum[-1, i]
    ...
    def __call__(self, signal):
        ...
        if np.random.uniform(0, 1) < self.p:
            ...
            if self.channels == 1:
                return np.squeeze(signal_new)
            # new_signal = np.stack((signal, signal_new), 0)
            return signal_new
        return signal
