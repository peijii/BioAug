...
class Permutation(object):
    ...
    Args:
        ...
        nPerm         (int)   : The number of segments into which the signal will be divided. The signal is split into 'nPerm'
                                segments, which are then randomly reordered.
    ...
    def __call__(self, signal):
        ...
        if np.random.uniform(0, 1) < self.p:
            ...
            while flag:
                ...
                if np.min(segs[1:] - segs[:-1]) > self.minSegLength:
                    flag = False
